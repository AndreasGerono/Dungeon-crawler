...
pub fn apply(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    let mut attempts = 0;
    while placment.is_none() && attempts < 10 {
        ...
        dimensions.for_each(|pt| {
            ...
            let distance = dijkstramap.map[idx];
            if (distance < 2000.0)
                && (distance > 20.0)
                && (mb.amulet_start != pt)
            {
        ...
        if can_place {
            placment = Some(Point::new(dimensions.x1, dimensions.y1));
    ...
    if let Some(placment) = placment {
        ...
        for ty in placment.y..placment.y + FORTRESS.2 {
            for tx in placment.x..placment.x + FORTRESS.1 {
